...
const roleProfiles={
  Presidencia:{subtitle:'Control del club · datos vivos en Supabase',focus:'Inteligencia del club'},
...
function friendlyError(error){
  ...
  if(/not authorized/i.test(raw))return 'Tu usuario no tiene permiso para esta sección.';
  return raw;
}
...
async function rpcSafe(name,params,fallback,timeoutMs=7000){
  let timer;try{return await Promise.race([rpc(name,params),new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error(`${name} timeout`)),timeoutMs);})]);}
  catch(error){console.warn('home rpc',name,error);return fallback;}finally{if(timer)clearTimeout(timer);}
}
...
function renderImmediateHome(){
  ...
  if($('attentionList'))$('attentionList').innerHTML='<article class="tos-alert good"><span class="tos-alert-tag">TannerOS</span><b>Módulos conectados</b><small>Los datos se actualizan en segundo plano.</small></article>';
  if($('agendaList'))$('agendaList').innerHTML='<div class="tos-empty">Cargando agenda…</div>';
  if($('roleFocusBody'))$('roleFocusBody').innerHTML='<div class="tos-empty">Cargando inteligencia del club…</div>';
...
async function loadAuthenticatedApp(){
  ...
  try{
    const {data:{user}}=await supabase.auth.getUser();if(!user){showView('authView');document.body.classList.remove('tos-body');return;}
    ...
    ctx=rows[0];navigation=await rpc('v2_my_navigation',{organization_id:ctx.organization_id});
    ...
    loadHomeData().catch(error=>{console.error('home data',error);setShellHealth({state:'attention',label:'Datos parciales'});});
...
async function loadHomeData(){
  ...
  await Promise.allSettled(jobs);renderHome();
}
...
function renderAttention(){
  ...
  if($('attentionList'))$('attentionList').innerHTML=alerts.slice(0,6).join('');
...
function renderAgenda(){
  const rows=[...state.calendar].sort((a,b)=>new Date(a.startsAt||a.starts_at)-new Date(b.startsAt||b.starts_at)).slice(0,6);$('calendarLink')?.classList.toggle('hidden',!can('calendario'));if($('calendarLink'))$('calendarLink').href='/calendario/';
...
function countBy(rows,keyFn){const map=new Map();for(const row of rows){const key=String(keyFn(row)||'Sin dato').trim()||'Sin dato';map.set(key,(map.get(key)||0)+1);}return [...map.entries()].sort((a,b)=>b[1]-a[1]);}
function mini(label,value,sub=''){return `<div class="tos-mini-stat"><strong>${esc(value)}</strong><span>${esc(label)}</span>${sub?`<small>${esc(sub)}</small>`:''}</div>`;}
function renderRoleFocus(){
  const title=$('roleFocusTitle');if(title)title.textContent=(roleProfiles[ctx?.role]||roleProfiles.Presidencia).focus;
  const sourceTop=countBy(state.prospects,p=>p.source_campaign||p.source_channel||p.source)[0];
  const categoryTop=countBy(state.players,p=>p.category)[0];
  const collection=state.collection?`${state.collection.collection_rate||0}%`:'—';
  const html=`<div class="tos-mini-stats">${mini('Cobranza',collection,state.collection?`${state.collection.pending_players||0} pendientes`:'Sin dato')}${mini('Mejor fuente',sourceTop?.[0]||'Sin campañas',sourceTop?`${sourceTop[1]} registros`:'')}${mini('Categoría mayor',categoryTop?.[0]||'Sin categoría',categoryTop?`${categoryTop[1]} Tanners`:'')}</div>`;
  if($('roleFocusBody'))$('roleFocusBody').innerHTML=html;
  const link=$('roleFocusLink');if(link){link.classList.remove('hidden');link.href='/direccion/';link.textContent='Ver dirección';}
}
...
function renderBirthdays(){
  const rows=state.players.map(p=>({...p,days:daysToBirthday(p.birth_date)})).filter(p=>p.days>=0&&p.days<=31).sort((a,b)=>a.days-b.days).slice(0,8);$('birthdayPanel')?.classList.toggle('hidden',!rows.length);
...
function renderHome(){renderKpis();renderQuickActions();renderAttention();renderAgenda();renderRoleFocus();renderBirthdays();renderSearch();}

async function cleanupLegacyRuntime(){
  try{if('serviceWorker' in navigator){const regs=await navigator.serviceWorker.getRegistrations();await Promise.all(regs.map(r=>r.unregister()));}if('caches' in window){const keys=await caches.keys();await Promise.all(keys.filter(k=>/^tanneros-shell/i.test(k)||k==='app-shell').map(k=>caches.delete(k)));}}catch(error){console.warn('legacy cache cleanup',error);}
}
...
if(recovery){await renderRecovery();}
  ...
  const {data:{session}}=await supabase.auth.getSession();if(session)await loadAuthenticatedApp();else{showView('authView');document.body.classList.remove('tos-body');}
  supabase.auth.onAuthStateChange((event,sessionNow)=>{if(event==='SIGNED_IN'&&sessionNow)setTimeout(()=>loadAuthenticatedApp(),0);if(event==='SIGNED_OUT'){ctx=null;navigation=[];state={players:[],prospects:[],calendar:[],attendance:[],sponsors:[],equipment:[],collection:null};showView('authView');document.body.classList.remove('tos-body');}});
}

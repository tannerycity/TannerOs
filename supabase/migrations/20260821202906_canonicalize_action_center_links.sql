...
begin
  ...
  if private.has_module_access(p_organization_id,'billing',false) then
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' cargo(s) vencido(s)','count',v_count,'amount',v_amount,'href','/finanzas/'
      ));
    end if;
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' perfil(es) requieren configuración o revisión','count',v_count,'href','/finanzas/'
      ));
    end if;
  end if;
  ...
  if private.has_module_access(p_organization_id,'prospects',false) then
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' prospecto(s) tienen una próxima acción vencida','count',v_count,'href','/prospectos/'
      ));
    end if;
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' prospecto(s) nuevos aún no tienen seguimiento programado','count',v_count,'href','/prospectos/'
      ));
    end if;
  end if;
  ...
  if private.has_module_access(p_organization_id,'commerce',false) then
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' pedido(s) están listos','count',v_count,'href','/pedidos/'
      ));
    end if;
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' pedido(s) siguen pendientes de pago','count',v_count,'href','/pedidos/'
      ));
    end if;
  end if;
  ...
  if private.has_module_access(p_organization_id,'sponsors',false) then
    ...
    if coalesce(v_sponsor_overdue,0)>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_sponsor_overdue||' patrocinador(es) tienen convenio vencido','count',v_sponsor_overdue,'href','/patrocinadores/'
      ));
    end if;
    if coalesce(v_sponsor_soon,0)>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_sponsor_soon||' patrocinador(es) están en ventana de renovación','count',v_sponsor_soon,'href','/patrocinadores/'
      ));
    end if;
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' marca(s) tienen próxima acción vencida','count',v_count,'href','/patrocinadores/'
      ));
    end if;
  end if;
  ...
  if private.has_module_access(p_organization_id,'equipment',false) then
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' artículo(s) requieren reposición','count',v_count,'href','/utileria/'
      ));
    end if;
  end if;
  ...
  if private.has_module_access(p_organization_id,'players',false) then
    ...
    if v_count>0 then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        ...
        'detail',v_count||' jugador(es) activos están marcados para revisión','count',v_count,'href','/jugadores/'
      ));
    end if;
  end if;
  ...
  if private.has_module_access(p_organization_id,'admin',false) then
    ...
    if coalesce(v_org_settings->>'legacyCutoverStatus','') <> 'complete' then
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        'code','legacy_cutover','priority','attention','module','admin','title','Corte del sistema anterior pendiente',
        'detail','El sistema anterior aún no está marcado como retirado','href','/admin/onboarding/'
      ));
    end if;
  end if;

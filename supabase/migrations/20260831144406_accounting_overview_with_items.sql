...
begin
  ...
  -- Ingresos por categoría (mes) CON desglose de movimientos
  ...
  from (
    select coalesce(nullif(category,''),'Sin categoría') cat, sum(amount) amt, count(*) cnt,
      jsonb_agg(jsonb_build_object(
        ...
        'who',coalesce(nullif(payer_name,''),'—'),
        ...
        'amount',amount
      ) order by payment_date desc, amount desc) items
    from app.payments
    where organization_id=p_organization_id and status='posted' and payment_date between s and e
    group by coalesce(nullif(category,''),'Sin categoría')
  ) t;

  -- Egresos por categoría (mes) CON desglose
  ...
  from (
    select coalesce(nullif(category,''),'Sin categoría') cat, sum(amount) amt, count(*) cnt,
      jsonb_agg(jsonb_build_object(
        'date',expense_date,
        'concept',coalesce(nullif(concept,''),'—'),
        'who',coalesce(nullif(supplier_name,''),'—'),
        'method',case lower(coalesce(method,'')) when 'cash' then 'Efectivo' when 'efectivo' then 'Efectivo' when 'transfer' then 'Transferencia' when 'transferencia' then 'Transferencia' when 'card' then 'Tarjeta' when 'tarjeta' then 'Tarjeta' else coalesce(nullif(method,''),'Otro') end,
        'amount',amount
      ) order by expense_date desc, amount desc) items
    from app.expenses
    where organization_id=p_organization_id and status='posted' and expense_date between s and e
    group by coalesce(nullif(category,''),'Sin categoría')
  ) t;
  ...
  return jsonb_build_object(
    ...
    'trend', v_trend, 'incomeByCategory', v_inc, 'expenseByCategory', v_exp, 'quality', v_qual, 'pnlBySource', v_pnl,

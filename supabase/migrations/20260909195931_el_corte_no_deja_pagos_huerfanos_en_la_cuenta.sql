-- Con el corte, un pago posterior al 1 de agosto que cubrio cargos ANTERIORES
-- quedaba visible sin sus cargos: la familia leia un abono suelto y parecia
-- saldo a favor. El pago de $1,600 de Liam es el caso: cubrio junio y julio,
-- que ya no se muestran.
--
-- De un pago de mensualidad solo se muestra lo que toca al periodo visible: lo
-- aplicado a cargos posteriores al corte mas lo que siga disponible. Si eso da
-- cero, el pago pertenece al pasado cerrado y no se pinta.
--
-- Los pagos de otros conceptos (tienda, inscripcion, academia) se muestran tal
-- cual: no viven en charges y su monto no se reparte entre periodos.
...
begin
  ...
  with movimientos as (
    ...
    union all
    select pm.payment_date, 2, 'payment', pm.payment_purpose, 'Pago recibido', -visible.monto,
           null, pm.status, null::date, pm.method, pm.id, null::text
    from app.payments pm
    cross join lateral (
      select case when pm.payment_purpose <> 'billing' then pm.amount
             else coalesce((
                    select sum(pa.amount) from app.payment_allocations pa
                    join app.charges c on c.id = pa.charge_id
                    where pa.payment_id = pm.id and pa.status = 'posted'
                      and coalesce(c.billing_period, c.due_date) >= v_corte), 0)
                + coalesce((select pb.available_credit from app.payment_balances pb
                            where pb.id = pm.id), 0)
             end as monto
    ) visible

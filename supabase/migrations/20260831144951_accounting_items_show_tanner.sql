...
begin
  ...
  -- Ingresos por categoría CON desglose: 'who' = Tanner (si está ligado) -> si no, quien pagó
  ...
  from (
    select coalesce(nullif(p.category,''),'Sin categoría') cat, sum(p.amount) amt, count(*) cnt,
      jsonb_agg(jsonb_build_object(
        'date',p.payment_date,
        'concept',coalesce(nullif(p.concept,''),'—'),
        ...
        'amount',p.amount
      ) order by p.payment_date desc, p.amount desc) items
    from app.payments p

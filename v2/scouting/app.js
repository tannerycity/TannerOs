...
function fmtBirthDate(v){if(!v)return '—';return new Intl.DateTimeFormat('es-MX',{dateStyle:'long'}).format(new Date(`${v}T12:00:00`));}
...
function highInterest(v){return ['alto','alta','high'].includes(String(v||'').trim().normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase());}
...
function filtered(){
  const status=$('statusFilter').value,pipeline=$('pipelineFilter').value,q=$('searchScout').value.trim().normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
  return reports.filter(r=>{
    if(status&&r.status!==status)return false;
    if(!pipelineMatch(r,pipeline))return false;
    if(!q)return true;
    const hay=[r.observed_name,r.player_position,r.category,r.observed_location,r.verdict,r.interest_level,r.guardian_name,r.contact_phone,r.star_quality].filter(Boolean).join(' ').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
...
function openCreate(prospect=null){
  ...
  ['observedName','observedLocation','playerPosition','category','technicalScore','physicalScore','tacticalScore','mentalScore','starQuality','createVerdict','createNotes','birthDate','residence','dominantFoot','heightCm','guardianName','contactPhone'].forEach(id=>$(id).value='');
...
function facts(r){
  ...
  const rows=[
    ['Contacto',r.contact_phone],['Tutor',r.guardian_name],['Edad',a==null?null:`${a} años`],['Fecha de nacimiento',r.birth_date?fmtBirthDate(r.birth_date):null],['Residencia',r.residence],['Pie hábil',r.dominant_foot],['Estatura',r.height_cm==null?null:`${Number(r.height_cm)} cm`],['Posición',r.player_position],['Categoría',r.category],['Lugar de visoría',r.observed_location],['Diferenciadores',r.star_quality],['Origen',r.prospect_id?'Prospecto TannerOS':(r.source||'Visoría directa')],['Próxima acción',r.next_action_at?fmtDate(r.next_action_at):'Sin programar'],['Resultado',r.player_id?'Fichado como jugador':null]
...
function openEdit(){
  ...
  $('drawerEyebrow').textContent='EDITAR VISORÍA';$('drawerName').textContent=r.observed_name||'Sin nombre';$('drawerMeta').textContent='Actualiza la ficha completa del jugador';
  ...
  $('editPlayerPosition').value=r.player_position||'';$('editCategory').value=r.category||'';$('editBirthDate').value=r.birth_date||'';$('editResidence').value=r.residence||'';$('editDominantFoot').value=r.dominant_foot||'';$('editHeightCm').value=r.height_cm??'';
...
async function saveScoutEdit(){
  if(!current||!canWrite)return;
  const name=$('editObservedName').value.trim(),observedAt=$('editObservedAt').value,birthDate=$('editBirthDate').value,scores=['editTechnicalScore','editPhysicalScore','editTacticalScore','editMentalScore'].map(id=>score($(id).value)),height=score($('editHeightCm').value);
  ...
  if(birthDate&&new Date(`${birthDate}T12:00:00`)>new Date()){message('editMessage','La fecha de nacimiento no puede estar en el futuro.');$('editBirthDate').focus();return;}
  ...
  try{
    await rpc('v2_edit_scouting_report',{
      ...
      birth_date:birthDate||null,contact_phone:$('editContactPhone').value.trim()||null,guardian_name:$('editGuardianName').value.trim()||null,dominant_foot:$('editDominantFoot').value||null,height_cm:height,residence:$('editResidence').value.trim()||null
    });
...
async function saveScout(){
  ...
  try{
    ...
    await rpc('v2_edit_scouting_report',{organization_id:ctx.organization_id,report_id:id,observed_name:name,observed_at:toIsoOrNull($('observedAt').value),observed_location:$('observedLocation').value.trim()||null,player_position:$('playerPosition').value.trim()||null,category:$('category').value.trim()||null,technical_score:scores[0],physical_score:scores[1],tactical_score:scores[2],mental_score:scores[3],star_quality:$('starQuality').value.trim()||null,verdict:$('createVerdict').value.trim()||null,notes:$('createNotes').value.trim()||null,status,interest_level:interest,next_action_at:next,birth_date:$('birthDate').value||null,contact_phone:$('contactPhone').value.trim()||null,guardian_name:$('guardianName').value.trim()||null,dominant_foot:$('dominantFoot').value||null,height_cm:$('heightCm').value?Number($('heightCm').value):null,residence:$('residence').value.trim()||null});
...
function mountPhotoUi(){
  ...
  if(identity&&!$('residence')){
    const label=document.createElement('label');label.innerHTML='Lugar de residencia<input id="residence" type="text" maxlength="160" autocomplete="address-level2" placeholder="Colonia, ciudad o municipio">';$('birthDate')?.closest('label')?.after(label);
  }
  ...
  if(detail&&!$('editSection')){
    const section=document.createElement('section');section.id='editSection';section.className='drawer-section scouting-editor hidden';section.innerHTML=`
      ...
      <div class="form-grid editor-grid">
        ...
        <label>Lugar de residencia<input id="editResidence" type="text" maxlength="160" autocomplete="address-level2" placeholder="Colonia, ciudad o municipio"></label>
        ...
        <label>Teléfono del tutor<input id="editContactPhone" type="tel" maxlength="20" inputmode="tel"></label>
        <label>Fecha y hora de visoría<input id="editObservedAt" type="datetime-local"></label>
        <label>Lugar de visoría<input id="editObservedLocation" type="text" maxlength="160" placeholder="Cancha, torneo o escuela"></label>
      </div>

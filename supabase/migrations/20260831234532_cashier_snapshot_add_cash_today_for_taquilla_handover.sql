...
-- Taquilla necesita UN número operativo: cuánto efectivo tiene hoy para entregar al cerrar turno.
-- No es el ledger completo (eso sigue oculto), es solo el neto de efectivo del día seleccionado,
-- sin desglose por método, sin movimientos, sin nadie más. Se calcula siempre (barato) y se
-- expone tanto en la vista completa (Presidencia/Contabilidad/Operaciones) como en la redactada (Taquilla).
...
begin
  ...
  -- Efectivo neto SOLO del día seleccionado (para entregar al cierre). Se calcula para todos.
  select

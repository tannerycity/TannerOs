...
async function renderFinance(){
  ...
  const _ovd=vencidos.filter(r=>!esBaja(r)&&!esResiduo(r));
  const bajas=vencidos.filter(esBaja),residuo=vencidos.filter(r=>!esBaja(r)&&esResiduo(r));
  const suma=list=>list.reduce((a,r)=>a+Number(r.balance_due||0),0);
  const abiertos=(receivables||[]).filter(r=>Number(r.balance_due||0)>0);
  const cobrables=abiertos.filter(r=>!esBaja(r)&&!esResiduo(r));
  ...
  const bajasSet=new Set(bajas.map(r=>r.player_id||r.player_name)),residuoSet=new Set(residuo.map(r=>r.player_id||r.player_name));const _byP={};_ovd.forEach(r=>{const k=r.player_id||r.player_name;_byP[k]=(_byP[k]||0)+1;});const _multi=Object.values(_byP).filter(n=>n>=2).length;const _rate=Number((collection&&collection.collection_rate)||0);const _alerts=[];
  const _aparte=[];
  if(bajasSet.size)_aparte.push({t:`${bajasSet.size} Tanner${bajasSet.size>1?'es':''} dado${bajasSet.size>1?'s':''} de baja con saldo`,d:`${money.format(suma(bajas))} · fuera de cobranza. Cancélalo con un ajuste autorizado para que deje de contar.`,cta:'Ajustar'});
  if(residuoSet.size)_aparte.push({t:`${residuoSet.size} con saldo anterior a septiembre`,d:`${money.format(suma(residuo))} · cargos migrados: el sistema no les aplica pagos solo. Concílialos uno por uno.`,cta:'Conciliar'});if(_ovdSet.size>0)_alerts.push({t:`${_ovdSet.size} Tanner${_ovdSet.size>1?'s':''} con pagos vencidos`,d:`${money.format(_ovdTotal)} por cobrar de Tanners activos`,tone:'danger'});if(_multi>0)_alerts.push({t:`${_multi} Tanner${_multi>1?'s':''} con 2+ meses vencidos`,d:'Prioriza el cobro por antigüedad',tone:'danger'});if(_rate>0&&_rate<85)_alerts.push({t:`Cobranza en ${_rate}%`,d:'Por debajo del objetivo (85%)',tone:'warn'});const _ajusteHref=can('contabilidad')?'/contabilidad/#ajustes':'';const _aparteRow=a=>`<span style="display:flex;flex-direction:column;gap:2px"><strong style="color:#4a585e;font-size:13.5px;font-weight:600">${esc(a.t)}</strong><span style="color:#7d8a8f;font-size:12px;line-height:1.4">${esc(a.d)}</span></span>${_ajusteHref?`<span style="color:#4a585e;font-weight:600;font-size:13px;white-space:nowrap">${esc(a.cta)} \u2192</span>`:''}`;const _aparteStyle='display:flex;justify-content:space-between;align-items:center;gap:12px;padding:11px 14px;border-radius:12px;background:#f4f6f5;border:1px solid #e2e7e5;text-decoration:none';const _aparteHtml=_aparte.length?`<div style="display:flex;flex-direction:column;gap:8px;margin-top:8px"><span style="color:#8a949a;font-size:11.5px;font-weight:600;letter-spacing:.02em">FUERA DE LA COBRANZA DEL MES</span>${_aparte.map(a=>_ajusteHref?`<a href="${_ajusteHref}" style="${_aparteStyle}">${_aparteRow(a)}</a>`:`<div style="${_aparteStyle}">${_aparteRow(a)}</div>`).join('')}</div>`:'';

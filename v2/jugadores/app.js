...
function setMetric(scoreId,barId,value,max=5){const n=num(value),ratio=n==null?0:Math.max(0,Math.min(max,n))/max,label=n==null?'Sin evidencia':ratio<=.2?'Necesita apoyo':ratio<=.4?'En proceso':ratio<=.6?'Esperado':ratio<=.8?'Sólido':'Destacado',score=$(scoreId),row=score.closest('.metric-row');score.textContent=n==null?'Sin evidencia':`${n}/${max} · ${label}`;row.dataset.level=n==null?'empty':ratio<=.4?'support':ratio<=.6?'expected':ratio<=.8?'solid':'standout';$(`${barId}`)?.style.setProperty('width',`${ratio*100}%`);}
function radarPoint(value,angle,max=5){const n=Math.max(0,Math.min(max,num(value)??0))/max,r=96*n,cx=130,cy=120,a=(angle-90)*Math.PI/180;return [cx+r*Math.cos(a),cy+r*Math.sin(a)];}
function setRadar(scores,max=5){document.querySelectorAll('#sportsRadar line').forEach(line=>{line.setAttribute('x1','130');line.setAttribute('y1','120');});const values=[scores?.tecnica,scores?.inteligencia,scores?.intensidad],angles=[0,120,240],points=values.map((v,i)=>radarPoint(v,angles[i],max));$('sportsRadarPolygon').setAttribute('points',points.map(p=>p.map(x=>x.toFixed(1)).join(',')).join(' '));points.forEach((p,i)=>{const dot=$(`radarPoint${i+1}`);dot.setAttribute('cx',p[0].toFixed(1));dot.setAttribute('cy',p[1].toFixed(1));});}
...
function renderSports(data){const summary=data?.summary||{},methodologyEvaluations=(data?.evaluations||[]).filter(e=>evaluationMeta(e)?.methodology_version==='TC_1.0'),latest=methodologyEvaluations[0]||null,s=latest?.scores||{},hasEval=!!latest,hasMatches=Number(summary.played||0)>0;$('sportsLoading').classList.add('hidden');$('sportsEmpty').classList.toggle('hidden',hasEval||hasMatches);$('sportsContent').classList.toggle('hidden',!hasEval&&!hasMatches);$('sportsPlayed').textContent=Number(summary.played||0);$('sportsMinutes').textContent=Number(summary.minutes||0);$('sportsGoals').textContent=Number(summary.goals||0);$('sportsAssists').textContent=Number(summary.assists||0);if(!hasEval){setCardSports({});$('evaluationDate').textContent='Sin evaluación';$('profileStage').textContent=current?.player?.category||'Etapa actual';$('profileMethod').textContent='Sin evaluación';setRadar({});['Technique','Game','Body','Mentality','Values'].forEach(k=>{$(`score${k}`).textContent='—';$(`bar${k}`).style.width='0%';});$('goalkeeperMetrics').classList.add('hidden');$('evaluationObjectives').classList.add('hidden');return;}const meta=evaluationMeta(latest),scaleMax=meta?.methodology_version==='TC_1.0'?5:10;setCardSports(s);$('evaluationDate').textContent=`Evaluación ${formatEvalDate(latest.date||latest.evaluatedOn||latest.evaluated_on)}`;$('profileStage').textContent=meta?.category||current?.player?.category||'Etapa actual';$('profileMethod').textContent=scaleMax===5?'TC 1.0 · escala 1–5':'Evaluación histórica · escala 1–10';setMetric('scoreTechnique','barTechnique',s.tecnica,scaleMax);setMetric('scoreGame','barGame',s.inteligencia,scaleMax);setMetric('scoreBody','barBody',s.intensidad,scaleMax);setMetric('scoreMentality','barMentality',s.mentalidad,scaleMax);setMetric('scoreValues','barValues',s.valores,scaleMax);setRadar(s,scaleMax);renderGoalkeeper(s);const previous=methodologyEvaluations[1],previousMeta=evaluationMeta(previous),newStage=meta?.category&&previousMeta?.category&&meta.category!==previousMeta.category;const goals=[meta?.strength?`<div><span>Fortaleza</span><strong>${esc(meta.strength)}</strong></div>`:'',meta?.priority?`<div><span>Prioridad actual</span><strong>${esc(meta.priority)}</strong></div>`:'',meta?.superpower?`<div><span>Superpoder</span><strong>${esc(meta.superpower)}</strong></div>`:'',newStage?`<div><span>Evolución</span><strong>Inicio de nueva etapa · ${esc(meta.category)}</strong></div>`:'',latest.sportsObjective&&!meta?`<div><span>Objetivo deportivo</span><strong>${esc(latest.sportsObjective)}</strong></div>`:'',latest.formativeObjective?`<div><span>Objetivo formativo</span><strong>${esc(latest.formativeObjective)}</strong></div>`:''].filter(Boolean).join('');$('evaluationObjectives').classList.toggle('hidden',!goals);$('evaluationObjectives').innerHTML=goals;}
const PROFILE_METHOD='TC_1.0';
const PROFILE_DIMENSIONS=[
  ['tecnica','Técnica','Tengo herramientas','Control, conducción, pase, golpeo y dominio de balón.'],
  ['inteligencia','Juego','Entiendo y resuelvo','Percepción, decisiones, ubicación, compañeros y uso del espacio.'],
  ['intensidad','Cuerpo','Puedo ejecutar','Coordinación, movilidad, equilibrio, agilidad y control corporal.'],
  ['mentalidad','Mentalidad','No desaparezco','Reacción al error, concentración, resiliencia, valentía y autonomía.'],
  ['valores','Espíritu','Represento algo más grande que yo','Respeto, compañerismo, humildad, disciplina y pertenencia.']
];
...
function evaluationScale(key){const scale=[[1,'Necesita apoyo'],[2,'En proceso'],[3,'Esperado'],[4,'Sólido'],[5,'Destacado'],['','Sin evidencia']];return scale.map(([value,label])=>`<label class="profile-eval-score${value===''?' no-evidence':''}"><input type="radio" name="profile-${key}" data-profile-score="${key}" value="${value}"><b>${value||'—'}</b><small>${label}</small></label>`).join('');}
function openInlineEvaluation(){
  ...
  box.innerHTML=`<div class="profile-eval-head"><div><div class="eyebrow">METODOLOGÍA TANNERY CITY · ${PROFILE_METHOD}</div><h3 id="inlineEvaluationTitle">Evaluar a ${esc([p.firstName,p.lastName].filter(Boolean).join(' '))}</h3><p>${esc(p.category||'Sin categoría')} · ${esc(period)}</p></div><button id="closeInlineEvaluation" class="secondary mini" type="button">Cerrar</button></div><form id="inlineEvaluationForm">${PROFILE_DIMENSIONS.map(([key,name,claim,help])=>`<fieldset class="profile-eval-dimension"><legend><strong>${name}</strong><span>${claim}</span></legend><div class="profile-eval-scale">${evaluationScale(key)}</div><details><summary>ⓘ ¿Qué observar?</summary><p>${help} Observa lo esperado para ${esc(p.category||'su etapa')}.</p></details></fieldset>`).join('')}<div class="profile-eval-decisions"><label>Fortaleza principal<small>¿Qué está haciendo especialmente bien?</small><select id="profileStrength" required>${profileOptionList(PROFILE_OPTIONS,true)}</select></label><label>Prioridad de desarrollo<small>¿Qué queremos ayudarle a mejorar?</small><select id="profilePriority" required>${profileOptionList(PROFILE_OPTIONS,true)}</select></label><label>Superpoder<small>No es una sexta calificación.</small><select id="profilePower">${profileOptionList(PROFILE_POWERS)}</select></label><label id="profilePowerOtherWrap" class="hidden">¿Cuál?<input id="profilePowerOther" maxlength="60"></label></div><details class="profile-eval-optional"><summary>Objetivos y nota opcional</summary><label>Próximo objetivo deportivo<input id="profileSportsObjective" maxlength="160"></label><label>Próximo objetivo formativo<input id="profileFormativeObjective" maxlength="160"></label><label>Nota interna<textarea id="profileEvalNotes" rows="3" maxlength="500"></textarea></label></details><div id="profileEvalMessage" class="inline-message hidden"></div><small id="profileEvalDraftStatus" class="profile-eval-draft">Los cambios se guardan en este dispositivo.</small><div class="profile-eval-save"><button id="profileSaveDraft" class="secondary" type="button">Guardar borrador</button><button class="primary" type="submit">Guardar Perfil Tanner</button></div></form>`;
  ...
  $('inlineEvaluationForm').addEventListener('input',saveInlineDraft);$('inlineEvaluationForm').addEventListener('submit',saveInlineEvaluation);$('profileSaveDraft').addEventListener('click',saveInlineDraft);$('profilePower').addEventListener('change',e=>$('profilePowerOtherWrap').classList.toggle('hidden',e.target.value!=='Otro'));$('closeInlineEvaluation').addEventListener('click',closeInlineEvaluation);
}

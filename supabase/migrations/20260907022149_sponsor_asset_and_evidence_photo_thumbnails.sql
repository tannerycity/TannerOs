-- 1. Thumbnail columns
alter table app.sponsor_assets add column if not exists photo_thumb_path text;
alter table app.sponsor_agreement_item_evidence add column if not exists photo_thumb_path text;

-- 2. command_set_sponsor_asset_photo (adds optional thumb path)
drop function if exists private.command_set_sponsor_asset_photo(uuid,uuid,text);
drop function if exists public.v2_set_sponsor_asset_photo(uuid,uuid,text);

create function private.command_set_sponsor_asset_photo(p_organization_id uuid, p_asset_id uuid, p_photo_path text, p_thumb_path text default null)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_thumb_path is not null then
    v_thumb_parts := string_to_array(p_thumb_path, '/');
    if coalesce(array_length(v_thumb_parts,1),0) <> 6
       or v_thumb_parts[1] <> 'organizations'
       or v_thumb_parts[2] <> p_organization_id::text
       or v_thumb_parts[3] <> 'sponsors'
       or v_thumb_parts[4] <> 'assets'
       or v_thumb_parts[5] <> p_asset_id::text
       or v_thumb_parts[6] !~* '^photo-[0-9]{10,16}-thumb\.(jpg|jpeg|png|webp)$'
    then
      raise exception 'Invalid photo path';
    end if;
  end if;
  ...
  if p_thumb_path is not null and not exists (
    select 1 from storage.objects o
    where o.bucket_id = 'tanneros-private' and o.name = p_thumb_path
  ) then
    raise exception 'Photo upload not found';
  end if;
  ...
  select jsonb_build_object(
    ...
    'availability',sa.availability,'photoBucket',sa.photo_bucket,'photoPath',sa.photo_path,'photoThumbPath',sa.photo_thumb_path,
...
create function public.v2_set_sponsor_asset_photo(organization_id uuid, asset_id uuid, photo_path text, thumb_path text default null)
returns jsonb
language sql
security definer
set search_path to 'pg_catalog','private'
as $function$
  select private.command_set_sponsor_asset_photo(organization_id, asset_id, photo_path, thumb_path)
$function$;

-- 3. command_add_sponsor_item_evidence (adds optional thumb path)
drop function if exists private.command_add_sponsor_item_evidence(uuid,uuid,text,text);
drop function if exists public.v2_add_sponsor_item_evidence(uuid,uuid,text,text);

create function private.command_add_sponsor_item_evidence(p_organization_id uuid, p_item_id uuid, p_photo_path text, p_note text default null, p_thumb_path text default null)
returns jsonb
language plpgsql
security definer
set search_path to 'pg_catalog','app','private','storage'
...
begin
  ...
  if p_thumb_path is not null then
    v_thumb_parts := string_to_array(p_thumb_path, '/');
    if coalesce(array_length(v_thumb_parts,1),0) <> 8
       or v_thumb_parts[1] <> 'organizations'
       or v_thumb_parts[2] <> p_organization_id::text
       or v_thumb_parts[3] <> 'sponsors'
       or v_thumb_parts[4] <> 'agreements'
       or v_thumb_parts[5] <> v_agreement_id::text
       or v_thumb_parts[6] <> 'items'
       or v_thumb_parts[7] <> p_item_id::text
       or v_thumb_parts[8] !~* '^evidence-[0-9]{10,16}-thumb\.(jpg|jpeg|png|webp)$'
    then
      raise exception 'Invalid evidence path';
    end if;
  end if;
  ...
  insert into app.sponsor_agreement_item_evidence(
    organization_id, item_id, photo_bucket, photo_path, photo_thumb_path, note, uploaded_by_user_id
  ) values (
    p_organization_id, p_item_id, 'tanneros-private', p_photo_path, p_thumb_path, nullif(trim(coalesce(p_note,'')),''), v_actor
  ) returning * into v_row;

  return jsonb_build_object(
    'id',v_row.id,'itemId',v_row.item_id,'photoBucket',v_row.photo_bucket,'photoPath',v_row.photo_path,'photoThumbPath',v_row.photo_thumb_path,
    'note',v_row.note,'createdAt',v_row.created_at
  );
end
$function$;

create function public.v2_add_sponsor_item_evidence(organization_id uuid, item_id uuid, photo_path text, note text default null, thumb_path text default null)
returns jsonb
language sql
security definer
set search_path to 'pg_catalog','private'
as $function$
  select private.command_add_sponsor_item_evidence(organization_id, item_id, photo_path, note, thumb_path)
$function$;

-- 4. Extend query_sponsor_admin to expose the thumb paths
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',e.id,'itemId',e.item_id,'photoBucket',e.photo_bucket,'photoPath',e.photo_path,'photoThumbPath',e.photo_thumb_path,
...
-- 5. Grants (mirror the originals: authenticated only)
revoke all on function private.command_set_sponsor_asset_photo(uuid,uuid,text,text) from public, anon, authenticated;
revoke all on function private.command_add_sponsor_item_evidence(uuid,uuid,text,text,text) from public, anon, authenticated;

revoke all on function public.v2_set_sponsor_asset_photo(uuid,uuid,text,text) from public, anon;
revoke all on function public.v2_add_sponsor_item_evidence(uuid,uuid,text,text,text) from public, anon;

grant execute on function public.v2_set_sponsor_asset_photo(uuid,uuid,text,text) to authenticated;
grant execute on function public.v2_add_sponsor_item_evidence(uuid,uuid,text,text,text) to authenticated;
;

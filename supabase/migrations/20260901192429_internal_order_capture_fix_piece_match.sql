-- Fix: los componentes de app.product_bundles guardan legacy_id en algunos kits viejos
-- (ej. "pro_mrr0dhu7_qvh0ex"), no el uuid real. El front (Catálogo) siempre manda el uuid
-- real por pieza, así que el emparejamiento debe hacerse contra el producto YA RESUELTO
-- (v_product.id), no contra el string crudo de components[].productId.

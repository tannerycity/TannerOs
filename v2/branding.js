...
export function applyBranding(raw,{organizationId}={}){
  ...
  let runtime=document.getElementById('tosBrandRuntime');if(!runtime){runtime=document.createElement('style');runtime.id='tosBrandRuntime';document.head.appendChild(runtime);}runtime.textContent=`.tos-nav-item.active{background:color-mix(in srgb,var(--tos-teal) 72%,var(--tos-ink-deep))!important}.tos-welcome,.brand-hero,.hero{background:#fff!important;color:#17262c!important}.tos-brand-mark{background:#f2f4f3!important;color:var(--tos-ink-deep)!important}.primary{background:var(--tos-ink-deep)}`;
  ...
  const icon180='/icon-180.png';
  const apple=ensureLink('apple-touch-icon','tosAppleTouchIcon');apple.href=icon180;apple.sizes='180x180';const fav=ensureLink('icon','tosFavicon');fav.href='/icon.svg';fav.type='image/svg+xml';
  ...
  const markUrl=brandingAssetUrl(brand.assets.mark);

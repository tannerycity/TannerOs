create or replace function app.withdraw_player(p_player_id uuid, p_withdrawn_at date, p_reason text, p_actor text default null::text)
returns void
language plpgsql
security definer
...
begin
  ...
  values(v_org,'PlayerWithdrawn','player',p_player_id,
         jsonb_build_object('withdrawn_at',p_withdrawn_at,'reason',trim(p_reason),'activeEnrollmentsClosed',true),p_actor);
end
$function$;;

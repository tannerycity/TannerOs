...
function moneyKpi(label,amount,sub=''){const numeric=Number(amount||0),cls=numeric>0?'danger':'good',badge=numeric>0?'Por cobrar':numeric<0?'A favor':'Al día',detail=numeric===0?'Sin saldo pendiente':numeric<0?'Saldo disponible':sub;return `<article class="tos-kpi tos-kpi-money ${cls}"><div class="tos-kpi-label"><span>${esc(label)}</span><em>${badge}</em></div><strong>${money.format(Math.abs(numeric))}</strong><small>${esc(detail)}</small></article>`;}

-- K3 · El cobro captura tambien el monto esperado y las observaciones.
--
-- El cuerpo se copia TAL CUAL del que corre hoy. Lo unico nuevo son
-- p_expected_amount y p_observations, y sus dos columnas en el insert.
-- En particular se conserva la linea
--     perform app.allocate_payment_oldest_first(v_id);
-- que es la que aplica el cobro a los adeudos del Tanner.
--
-- CUIDADO QUE ESTA MIGRACION SI TIENE
-- Anadir un parametro NO reemplaza la funcion: crea una segunda y deja la
-- vieja. Con DEFAULT, una llamada encaja con las dos y Postgres se niega a
-- elegir. Eso ya dejo a Taquilla sin poder registrar egresos una vez. Aqui
-- se crea Y se borra la anterior en la misma transaccion, y al final se
-- comprueba que quede UNA sola de cada una.
create or replace function private.command_post_payment(
  ...
  p_payer_name text, p_idempotency_key text, p_collected_by_name text,
  p_expected_amount numeric, p_observations text
)
returns uuid
language plpgsql
security definer
...
create or replace function public.v2_post_payment(
  organization_id uuid, player_id uuid, amount numeric, payment_date date,
  method text, reference text default null::text, concept text default null::text,
  payer_type text default 'guardian'::text, payer_name text default null::text,
  idempotency_key text default null::text, collected_by_name text default null::text,
  expected_amount numeric default null::numeric, observations text default null::text
) returns uuid
language sql
security definer
set search_path to 'pg_catalog', 'private'
as $function$
  select private.command_post_payment(organization_id,player_id,amount,payment_date,method,reference,concept,payer_type,payer_name,idempotency_key,collected_by_name,expected_amount,observations)
$function$;

-- Las versiones de 11 argumentos se van. Las firmas van completas a
-- proposito: sin ellas Postgres no sabria cual borrar.
drop function if exists public.v2_post_payment(uuid,uuid,numeric,date,text,text,text,text,text,text,text);
drop function if exists private.command_post_payment(uuid,uuid,numeric,date,text,text,text,text,text,text,text);

revoke all on function private.command_post_payment(uuid,uuid,numeric,date,text,text,text,text,text,text,text,numeric,text) from public, anon, authenticated;
revoke all on function public.v2_post_payment(uuid,uuid,numeric,date,text,text,text,text,text,text,text,numeric,text) from public, anon;
grant execute on function public.v2_post_payment(uuid,uuid,numeric,date,text,text,text,text,text,text,text,numeric,text) to authenticated;
...
begin
  ...
  -- Las lineas que no se pueden perder al recopiar el cuerpo.
  if not exists (
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='private' and p.proname='command_post_payment'
      and pg_get_functiondef(p.oid) like '%allocate_payment_oldest_first%'
  ) then raise exception 'Se perdio la aplicacion del pago a los adeudos'; end if;
  if not exists (
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='private' and p.proname='command_post_payment'
      and pg_get_functiondef(p.oid) like '%has_module_access(p_organization_id,''billing'',true)%'
  ) then raise exception 'Se perdio la validacion de permiso de cobro'; end if;
end $$;

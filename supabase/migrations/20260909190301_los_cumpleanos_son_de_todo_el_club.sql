-- Los cumpleanos van de todo el club, no solo de la categoria: es comunidad, y
-- es lo que un club publica igual en su grupo de WhatsApp.
--
-- PRIVACIDAD: se manda nombre, dia y mes. Nunca el ano ni la edad ni el
-- apellido completo: son menores, y felicitar a un companero no necesita su
-- fecha de nacimiento. Los de la categoria del hijo vienen marcados para que la
-- pantalla los pueda destacar.
...
begin
  ...
  with mis_categorias as (
    ...
    select pl.id, pl.first_name, pl.birth_date,
      (select c.name from app.player_enrollments pe join app.categories c on c.id=pe.category_id
        where pe.player_id=pl.id order by pe.starts_on desc nulls last limit 1) as categoria,
      exists (select 1 from app.player_enrollments pe
              where pe.player_id=pl.id and pe.category_id in (select category_id from mis_categorias)) as es_companero
    from app.players pl
    where pl.organization_id = g.organization_id
      and pl.status = 'active' and pl.archived_at is null
      and pl.birth_date is not null
  ), anos as (
    select generate_series(extract(year from p_from)::int, extract(year from p_to)::int) as ano
  ), fechas as (
    select k.id, k.first_name, k.categoria, k.es_companero,
      make_date(a.ano,
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'kind', 'birthday', 'mine', f.es_companero)

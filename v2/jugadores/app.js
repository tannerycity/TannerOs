...
function setMetric(scoreId,barId,value){const n=num(value);$(scoreId).textContent=n==null?'Sin evidencia':`${n}/5`;$(`${barId}`)?.style.setProperty('width',`${Math.max(0,Math.min(5,n??0))*20}%`);}
function radarPoint(value,angle){const n=Math.max(0,Math.min(5,num(value)??0))/5,r=96*n,cx=130,cy=120,a=(angle-90)*Math.PI/180;return [cx+r*Math.cos(a),cy+r*Math.sin(a)];}

...
function setRadar(scores,max=5){document.querySelectorAll('#sportsRadar line').forEach(line=>{line.setAttribute('x1','150');line.setAttribute('y1','135');});const values=[scores?.tecnica,scores?.inteligencia,scores?.intensidad,scores?.mentalidad,scores?.valores],angles=[0,72,144,216,288],points=values.map((v,i)=>radarPoint(v,angles[i],max));$('sportsRadarPolygon').setAttribute('points',points.map(p=>p.map(x=>x.toFixed(1)).join(',')).join(' '));points.forEach((p,i)=>{const dot=$(`radarPoint${i+1}`);dot.setAttribute('cx',p[0].toFixed(1));dot.setAttribute('cy',p[1].toFixed(1));});}

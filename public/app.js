...
async function renderRegistro(){
  ...
  show(`<div class="eyebrow">ÚNETE A TANNERY CITY</div><h2>Registro de jugador</h2><p class="muted">Déjanos los datos del jugador y de su tutor. Administración se pondrá en contacto contigo.</p>
  <form id="regForm" class="form-grid">
    <label>Nombre<input id="firstName" required></label><label>Apellidos<input id="lastName" required></label>
    <label>Fecha de nacimiento<input id="birthDate" type="date" required></label><label>Categoría de interés<select id="category"><option value="">Por definir</option><option>Baby Tanners</option><option>T6</option><option>T8</option><option>T10</option><option>T12</option></select></label>
    <label>Nombre del tutor<input id="guardian" required></label><label>Teléfono<input id="phone" inputmode="tel" required></label>
    <label>Correo<input id="email" type="email"></label><label>¿Cómo nos conociste?<select id="source"><option>Redes sociales</option><option>Recomendación</option><option>Evento</option><option>Cancha</option><option>Otro</option></select></label>
    <div class="span-2">${consent()}</div><div id="formMessage" class="message hidden span-2"></div><button class="primary span-2" type="submit">Enviar registro</button>
  </form>`);
  $('regForm').addEventListener('submit',async e=>{e.preventDefault();msg('');const btn=e.submitter;btn.disabled=true;try{await rpc('v2_public_register',{club_key:CLUB_KEY,first_name:$('firstName').value.trim(),last_name:$('lastName').value.trim(),birth_date:$('birthDate').value,phone:$('phone').value.trim(),email:$('email').value.trim()||null,guardian_name:$('guardian').value.trim(),category_interest:$('category').value||null,source_campaign:$('source').value,consent:{accepted:true,source:'public-web',at:new Date().toISOString()}});show(`<div class="success"><div class="success-mark">✓</div><h2>Registro recibido</h2><p>Gracias. Administración de Tannery City se pondrá en contacto contigo.</p></div>`);}catch(err){msg(err.message||'No se pudo enviar el registro.');btn.disabled=false;}});
}

async function renderPedido(){
  setTitle('Pedido Tanner');
  const products=await rpc('v2_public_products',{club_key:CLUB_KEY});
  const options=(products||[]).map(p=>`<option value="${p.id}" data-price="${p.price}">${p.name} · ${money.format(Number(p.price||0))}</option>`).join('');
  show(`<div class="eyebrow">TIENDA TANNER</div><h2>Levanta tu pedido</h2><p class="muted">Selecciona el producto y deja tus datos. Administración te contactará para confirmar pago, talla y entrega.</p>
  ...
  $('orderForm').addEventListener('submit',async e=>{e.preventDefault();msg('');const btn=e.submitter;btn.disabled=true;try{const productId=$('product').value;const qty=Number($('qty').value||1);const result=await rpc('v2_public_order',{club_key:CLUB_KEY,customer_name:$('customerName').value.trim(),customer_phone:$('customerPhone').value.trim(),customer_email:$('customerEmail').value.trim()||null,items:[{product_id:productId,quantity:qty}],notes:$('notes').value.trim()||null});show(`<div class="success"><div class="success-mark">✓</div><h2>Pedido recibido</h2><p>Tu pedido fue registrado correctamente.</p>${result?.folio?`<div class="folio">${result.folio}</div>`:''}</div>`);}catch(err){msg(err.message||'No se pudo enviar el pedido.');btn.disabled=false;}});
}
...
async function renderProgramas(){
  ...
  const cards=programs.map(p=>`<article class="program-card"><div class="eyebrow">${p.type||'Programa'}</div><h3>${p.name}</h3><p>${p.description||''}</p><button class="secondary enroll" data-slug="${p.slug}">Inscribirme</button></article>`).join('');show(`<div class="eyebrow">PROGRAMAS</div><h2>Inscripciones abiertas</h2><div class="programs">${cards}</div><div id="programFormWrap"></div>`);

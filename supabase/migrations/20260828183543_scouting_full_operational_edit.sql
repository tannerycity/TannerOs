...
begin
  ...
  return jsonb_build_object(
    ...
    'contact_phone', v_report.contact_phone,
    'guardian_name', v_report.guardian_name,
    'birth_date', v_report.birth_date,
...
create or replace function public.v2_edit_scouting_report(
  organization_id uuid,
  report_id uuid,
  observed_name text,
...
begin
  ...
  if $8 is not null and ($8 < 0 or $8 > 10) then
    raise exception 'Technical score out of range';
  end if;
  if $9 is not null and ($9 < 0 or $9 > 10) then
    raise exception 'Physical score out of range';
  end if;
  if $10 is not null and ($10 < 0 or $10 > 10) then
    raise exception 'Tactical score out of range';
  end if;
  if $11 is not null and ($11 < 0 or $11 > 10) then
    raise exception 'Mental score out of range';
  end if;
  if $22 is not null and ($22 < 0 or $22 > 230) then
...
revoke all on function public.v2_edit_scouting_report(
  uuid, uuid, text, timestamptz, text, text, text, numeric, numeric, numeric,
  numeric, text, text, text, text, text, timestamptz, date, text, text, text, numeric
) from public, anon;

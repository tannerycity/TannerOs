...
begin
  ...
  select case when jsonb_typeof(coalesce(v_org.branding->'assets','{}'::jsonb))='object' then jsonb_object_length(coalesce(v_org.branding->'assets','{}'::jsonb)) else 0 end into v_brand_assets;
...
$$;
revoke all on function private.query_onboarding_readiness(uuid) from public,anon;
grant execute on function private.query_onboarding_readiness(uuid) to authenticated;

create or replace function public.v2_onboarding_readiness(organization_id uuid)
returns jsonb language sql security invoker set search_path=pg_catalog,private
as $$ select private.query_onboarding_readiness(organization_id); $$;
revoke all on function public.v2_onboarding_readiness(uuid) from public,anon;
grant execute on function public.v2_onboarding_readiness(uuid) to authenticated;;

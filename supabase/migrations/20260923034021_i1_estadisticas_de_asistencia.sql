-- I1 · Estadisticas de asistencia
--
-- APLICADA dos veces, a proposito:
--   20260923032018  i1_estadisticas_de_asistencia
--   20260923034021  i1b_estadisticas_de_asistencia_con_comentarios
-- La primera se pego a mano y perdio los comentarios de adentro. La
-- segunda es identica en comportamiento y los devuelve, para que el SQL
-- que corre lleve su propia explicacion. Verificado con md5 que este
-- archivo y la base son iguales byte a byte.
--
-- Solo lectura. No toca ninguna tabla, no cambia ningun registro historico.
--
-- LAS TRES CUBETAS, Y POR QUE
-- Medido en produccion: de 328 pares (sesion x Tanner inscrito) solo 198
-- tienen registro. 130, el 39.6%, nadie los marco.
--
-- Por eso NO se puede contar "falta = programadas - presentes": le colgaria
-- 130 faltas a Tanners que quiza si fueron. Y tampoco basta "falta = registro
-- absent": eso infla el porcentaje ignorando que de 4 de cada 10 no se sabe.
--
-- Van tres cubetas separadas y el porcentaje se calcula SOLO sobre lo marcado:
--     asistio   = present + late   (llegar tarde es haber ido)
--     falta     = absent + excused (la justificada sigue siendo ausencia)
--     sin_marcar= no hay registro   (no cuenta para el porcentaje)
-- El "sin_marcar" se devuelve siempre, para que la pantalla pueda decir que
-- tan confiable es ese porcentaje.
--
-- EL ROSTER SE ESPEJEA DE private.session_roster_ids
-- Si esta consulta usara otra definicion de "quien debia estar", el reporte y
-- la lista de captura dirian cosas distintas del mismo dia. Se copia la rama
-- de categoria tal cual. Las sesiones de academia quedan fuera a proposito:
-- tienen su propio modulo y su propio candado.
--
-- CANDADOS QUE RESPETA
--   · has_module_access(...,'attendance',false)  para poder leer
--   · quien no es admin del club solo ve sus categorias asignadas
--     (private.my_category_ids), igual que query_attendance_categories
--   · las sesiones canceladas no entran nunca
--
-- OBJETIVOS
--   80% ordinario, 90% con beca. La beca se lee de app.player_benefits
--   activa y vigente; no se inventa ningun dato nuevo.
...
begin
  ...
  -- Misma regla que private.session_roster_ids, rama de categoria.
  roster as (
  ...
  -- Por jugador, para las listas de asistencia baja.
  por_jugador as (
  ...
  -- Promedio semanal y mensual por categoria: se saca el porcentaje de cada
  -- semana (o mes) y se promedian los periodos. Asi una semana con una sola
  -- sesion no pesa lo mismo que el mes entero.
  por_semana as (
...
-- Crear una funcion en private le devuelve EXECUTE a PUBLIC. Hay que revocar

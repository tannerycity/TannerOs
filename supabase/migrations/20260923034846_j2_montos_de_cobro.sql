-- J2 · Montos de cobro: cuanto cobrarle a cada Tanner
--
-- La fuente es la que ya cobra, no una nueva: billing_profiles para la cuota,
-- charge_balances para lo que se debe este periodo, player_benefits para la
-- condicion autorizada y categories.monthly_fee (J1) para el ordinario.
-- No se guarda ningun monto nuevo que pueda contradecir al oficial.
--
-- LA NOTA QUE SI PUEDE VER COBRANZA
-- player_benefits.notes trae el motivo economico de la beca: por que la
-- familia la pidio, que situacion tienen. Eso NO lo ve Taquilla. Se agrega
-- collection_note, que es lo unico que Presidencia autoriza a que cobranza
-- lea, tipo "cobrar solo los martes" o "paga la abuela".
alter table app.player_benefits
  add column if not exists collection_note text;

comment on column app.player_benefits.collection_note is
  'Lo unico de este beneficio que ve Taquilla. El motivo economico va en notes y nunca sale de Presidencia.';
...
declare
  ...
  v_out jsonb;
begin
  -- Cobranza vive en taquilla; Contabilidad y Presidencia tambien entran.
  ...
  benef as (
    select b.player_id,
           jsonb_agg(jsonb_build_object(
             'benefitId', b.id,
             'type', b.benefit_type,
             'label', coalesce(nullif(trim(b.legacy_label),''), private.etiqueta_de_beneficio(b.benefit_type)),
             'calculation', b.calculation_type,
             -- El desglose economico solo para quien puede verlo.
  ...
  from (
    select jsonb_build_object(
      ...
      'conditionLabel', case
        ...
        when b.n = 1 then coalesce(b.lista->0->>'label','Beneficio')
...
create or replace function public.v2_collection_amounts(
  organization_id uuid, billing_period date default null
) returns jsonb
language sql stable security definer set search_path to 'pg_catalog','private'
as $function$ select private.query_collection_amounts(organization_id, billing_period) $function$;

revoke all on function private.query_collection_amounts(uuid,date) from public, anon, authenticated;
revoke all on function public.v2_collection_amounts(uuid,date) from public, anon;
grant execute on function public.v2_collection_amounts(uuid,date) to authenticated;

do $$
declare v int;
begin
  select count(*) into v from pg_proc p join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='public' and p.proname='v2_collection_amounts';
  if v <> 1 then raise exception 'v2_collection_amounts quedo % veces', v; end if;
end $$;

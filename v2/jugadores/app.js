...
function setMetric(scoreId,barId,value,max=5,previous=null){const n=num(value),prev=num(previous),ratio=n==null?0:Math.max(0,Math.min(max,n))/max,label=n==null?'Sin evidencia':ratio<=.2?'Necesita apoyo':ratio<=.4?'En proceso':ratio<=.6?'Esperado':ratio<=.8?'Sólido':'Destacado',trend=n==null||prev==null?'':n>prev?' ↑':n<prev?' ↓':' →',score=$(scoreId),row=score.closest('.metric-row');score.textContent=n==null?'Sin evidencia':`${n}/${max} · ${label}${trend}`;row.dataset.level=n==null?'empty':ratio<=.4?'support':ratio<=.6?'expected':ratio<=.8?'solid':'standout';$(`${barId}`)?.style.setProperty('width',`${ratio*100}%`);}
...
const PROFILE_SCALE_LABELS={1:'Necesita apoyo',2:'En proceso',3:'Esperado',4:'Sólido',5:'Destacado','':'Sin evidencia'};
...
function updateInlineEvaluationProgress(){const total=PROFILE_DIMENSIONS.length,scores={};document.querySelectorAll('[data-profile-score]:checked').forEach(i=>scores[i.dataset.profileScore]=i.value);const done=Object.keys(scores).length;$('profileEvalProgressText').textContent=done===total?'Perfil completo':`${done} de ${total} dimensiones`;$('profileEvalProgressBar').style.width=`${done/total*100}%`;PROFILE_DIMENSIONS.forEach(([key])=>{const output=$(`profile-eval-selection-${key}`),value=scores[key];output.textContent=value===undefined?'Elige una opción':`${value||'?'} · ${PROFILE_SCALE_LABELS[value]}`;output.classList.toggle('selected',value!==undefined);});const angles=[0,72,144,216,288],points=PROFILE_DIMENSIONS.map(([key],i)=>radarPoint(scores[key],angles[i],5));$('profileEvalRadarPolygon').setAttribute('points',points.map(p=>p.map(x=>x.toFixed(1)).join(',')).join(' '));}

...
begin
  ...
  where organization_id=p_organization_id
    and payment_date=p_business_date
    and status='posted';
  ...
  from app.expenses
  where organization_id=p_organization_id
    and expense_date=p_business_date
    and status='posted';
  ...
  with movement_rows as (
    select p.id,'income'::text movement_type,p.payment_date movement_date,p.created_at,
      ...
      p.amount::numeric amount,p.status,p.source,p.reference
    from app.payments p
    ...
    select e.id,'expense',e.expense_date,e.created_at,
      ...
      e.amount::numeric,e.status,e.source,e.reference
    from app.expenses e
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'status',status,'source',source,'reference',reference
...
revoke all on function private.query_cashier_snapshot(uuid,date) from public,anon;
grant execute on function private.query_cashier_snapshot(uuid,date) to authenticated;

create or replace function public.v2_cashier_snapshot(
...
as $$
  select private.query_cashier_snapshot(organization_id,business_date);
$$;

revoke all on function public.v2_cashier_snapshot(uuid,date) from public,anon;
grant execute on function public.v2_cashier_snapshot(uuid,date) to authenticated;;

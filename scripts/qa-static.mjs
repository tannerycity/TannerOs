...
// original y el panel heredado no debe reactivar descargas periódicas.
...
const legacy=fs.readFileSync('tcfc-panel-academia-v1.html','utf8');
const autoSyncBlock=legacy.match(/function startAutoSync\(\)\{[\s\S]*?\n\}/)?.[0]||'';
if(/pullFromSheets\(/.test(autoSyncBlock))errors.push('Egress: el panel v1 mantiene el pull automático masivo');
